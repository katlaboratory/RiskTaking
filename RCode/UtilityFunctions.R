...
util.getAllDepVarsForLG<-function(data)
{
  res<-rep("", nrow(data))
  for(k in 1:nrow(data))
  {
    ...
    {
      if(data[[colname]][k] == "TRUE" & colname!= "(Intercept)")
      {
        if(nchar(indepVars)>1)
        {indepVars<-paste(indepVars , paste("+",colname,  sep=""), sep="")}
        else{indepVars<-colname}
      }
    }
    #print(paste(k, " : ", indepVars, sep=""))
    res[k]<-indepVars
  }
  return (res)
}
...
util.GetModelsSignificantTable<-function(dataIn, dependentVariablesNames , predictorVariables, pValueThresh=0.05)
{
  predVars2<-predictorVariables
  
  for(cnt in 1:length(predictorVariables))
  {
    if(predictorVariables[cnt]!="GROUP")
    {
      if(min(dataIn[[predictorVariables[cnt]]], na.rm=TRUE) == Inf & max(dataIn[[predictorVariables[cnt]]], na.rm=TRUE) == -Inf)
      {
        predVars2<-predVars2[!predVars2 %in% c(predictorVariables[cnt])]
      }
    }
  }
  
  modelsSignificant <- data.frame(matrix(ncol = 5, nrow = 0))
  colnames(modelsSignificant)<-c("LinearModel", "AdjR2_Pf", "VIF", "COR", "ToExclude")
  for(kk in 1:length(dependentVariablesNames))
  {
    depVariableName<-dependentVariablesNames[kk]
    outliersDf<-outliers.GetOulierSubjects(dataIn, c(depVariableName))
    ### remove outliers from dataframe
    dataIn<-dataIn[!(dataIn$SUBJ_ID %in% unique(outliersDf$SUBJ_ID)), ]
    
    formulaStr<-paste(depVariableName,"~",paste(predVars2, collapse = "+"),sep="")
    
    leaps1<-regsubsets(as.formula(formulaStr)
                       ,data=dataIn,nbest=20)
    sLeaps1<-summary(leaps1)
    dfSLeaps1<-as.data.frame(sLeaps1$which)
    stringDepVars1<-util.getAllDepVarsForLG(dfSLeaps1)
    groupLevels<-c("GROUPHC" , "GROUPRE", "GROUPAN")
    for(j in 1:length(stringDepVars1))
    {
      allVars<-unlist(strsplit(stringDepVars1[j], "+", fixed = TRUE))
      xxx<-allVars[allVars %in% groupLevels]; 
      if(length(xxx)>0)
      {
        allVars<-allVars[!allVars %in% groupLevels]
        allVars<-c(allVars, "GROUP")
      }
      f <- paste(depVariableName," ~ ", paste(allVars, collapse=" + "))
      
      tmp1<-dataIn
      for(jj in 1:length(allVars))
      {
        tmp1<-tmp1[!is.na(tmp1[[allVars[jj]]]) ,]
      }
      allLM<-lm(f, data=tmp1)
      pVal <- anova(allLM)$'Pr(>F)'[1]
      if(pVal<pValueThresh)
      {
        lmCoef<-summary(allLM)$coefficients
        #print(lmCoef)
        allVariablesAreSignificant<-TRUE
        
        pvs<-"HERE: "
        isGroupSignificant<-FALSE
        for(hh in 1:nrow(lmCoef))
        {
          if(!lmCoef[hh,1] %in% groupLevels)
          {next}
          
          if(lmCoef[hh,4]<=pValueThresh)
          {
            isGroupSignificant<-TRUE
          }
        }
        for(hh in 1:nrow(lmCoef))
        {
          if(lmCoef[hh,4]>pValueThresh)
          { 
            if(lmCoef[hh,1] %in% groupLevels & isGroupSignificant==TRUE)
            {next}
            
            allVariablesAreSignificant<-FALSE
            pvs<-paste(pvs, " " , stringDepVars1[j], "  ", lmCoef[hh,4])
          }
        }
        if(allVariablesAreSignificant == TRUE)
        {
          toPrint1<-paste(depVariableName, " ~ ", paste("(", round(lmCoef[,1 ],3),") * ", row.names(lmCoef), collapse="+" , sep=""), sep="")
          toPrint1<-str_replace(toPrint1, "\\* \\(Intercept\\)", "" )
          toPrint1<-gsub("*()", "", toPrint1)
          
          pVal <- anova(allLM)$'Pr(>F)'[1]
          toPrint2<-paste("adj R2: ", round(summary(allLM)$adj.r.squared,3), " Pr(>F):  ", round(pVal,4), sep="")
          ...
          if(nrow(lmCoef)>2)
          {
            sigVarDf <- data.frame(matrix(ncol = nrow(lmCoef)-1, nrow = nrow(tmp1)))
            colnames(sigVarDf)<-row.names(lmCoef)[-1]
            cnames<-colnames(sigVarDf)
            for(kj in 1:ncol(sigVarDf))
            {
              sigVarDf[[cnames[kj]]]<-tmp1[[cnames[kj]]]
            }
            for(kj in 1:ncol(sigVarDf))
            {
              sigVarDf<-sigVarDf[!is.na(sigVarDf[[cnames[kj]]]) ,]
            }
            res<-try(vifstep(sigVarDf, th=10))
            if(inherits(res, "try-error"))
            {
              vifPrintVIF<-"Error in vifstep. Check manually."
            }
            else
            {
              vfsModel<-vifstep(sigVarDf, th=10)
              tmpPrint<-paste(vfsModel@results$Variables, " VIF: ", round(vfsModel@results$VIF,3))
              vifPrintVIF<-paste(paste(tmpPrint, collapse = ", "), sep = "")
              vifPrintVIF<-str_trunc(vifPrintVIF, 90, "right")
            }
            res <- try(vfcModel<-vifcor(sigVarDf, th=0.4))
            if(inherits(res, "try-error"))
            {
              vifPrintCORR<-"Error in vifcor"
              vifToExclude<-"Probably there is high correlation. Check manually."
            }
            else
            {
              
              vfcModel<-vifcor(sigVarDf, th=0.4)
              m<-vfcModel@corMatrix
              
              dfTest<-data.frame(row=rownames(m)[row(m)[upper.tri(m)]], 
                                 col=colnames(m)[col(m)[upper.tri(m)]], 
                                 corr=m[upper.tri(m)])
              dfTest<-util.flattenCorrMatrix(m)
              vifPrintCORR<-paste(dfTest$row, " vs ",dfTest$col," : ", round(dfTest$cor,3))
              vifPrintCORR<-paste(paste(vifPrintCORR, collapse = ", "), sep = "")
              vifPrintCORR<-str_trunc(vifPrintCORR, 90, "right")
              if(length(vfsModel@excluded)>0 | length(vfcModel@excluded)>0)
              {
                vifToExclude<-paste("VIFstep: ",paste(vfsModel@excluded, collapse = ", "), " / VIFcorr: ", paste(vfcModel@excluded, collapse = ", "), sep = "")
              }
            }
          }
          printList<-list(LinearModel = toPrint1, AdjR2_Pf = toPrint2, VIF = vifPrintVIF, COR = vifPrintCORR, ToExclude = vifToExclude)
          modelsSignificant<-rbind(modelsSignificant, printList, stringsAsFactors=FALSE)
        }
      }
    }
  }
  return (modelsSignificant)
}


util.GenerateAndPrintLinearModels<-function(data , predictors , columnsGroup, fileSfx, excludeStudies=c(NA),  h=30, w=30 , pvThresh=0.05 , useStudyLevel=TRUE)
{
  #workingDataNotS4<-data[!(data$SUBJ_ID %in% unique(outliersDf$SUBJ_ID)) 
  #                       #& !(data$SUBJ_ID %in% stats.GetIDFromExclusionCriteria(data))
  #                       , ]
  if(!is.na(excludeStudies[1]))
  {
    data<-data[!data$STUDY %in% excludeStudies, ]
    
  }
  significantModels<-util.GetModelsSignificantTable(data, columnsGroup, predictors, pvThresh)
  if(nrow(significantModels)>0)
  {
    #require(gridExtra)
    #pdf(paste("N:\\Documents\\UCL\\WORK\\Projects\\Decision Making\\Risk Taking\\WorkingDataFiles\\OuputFiles\\SigRegressions", fileSfx ,".pdf" , sep=""),  height=h, width=w)
    #grid.table(significantModels)
    #dev.off()
  }
  else
  {print(paste("Dependent Variable Group ",fileSfx, " failed to produce any significant models.", sep="")  )}
...
util.GetAllSignificantPredictorsMLM<-function(data, depVarsGroup, useStudyLevel=TRUE, getInteractions =FALSE)
{
  modelsSignificant <- data.frame(matrix(ncol = 4, nrow = 0))
  colnames(modelsSignificant)<-c("DependentVariable", "Predictor", "P", "Slope")
 
  for ( cnt in 1:length(depVarsGroup))
  {
    prds<-util.GetAllSignificantPredictorsForOneDPMLM(data, depVarsGroup[cnt], useStudyLevel, getInteractions)
    #print(depVarsGroup[cnt]);print(prds)
    if(nrow(prds)==0)
    {next}
    for ( i in 1:nrow(prds))
    { #print(prds[i,]$Slope)
      prdList<-list(DependentVariable = depVarsGroup[cnt], Predictor = prds[i,]$Predictor, P= round(prds[i,][2],5), Slope= round(prds[i,][3],4))
      #print(prdList$P);print(prdList$Slope)
      modelsSignificant<-rbind(modelsSignificant, prdList, stringsAsFactors=FALSE)
     # print("here")
    }
  }
  modelsSignificant$P<-as.numeric(modelsSignificant$P)
  modelsSignificant$Slope<-as.numeric(modelsSignificant$Slope)
  return(modelsSignificant)
}

util.printDataRowMeanSD<-function(ffile , data, variableName, cTotal)
{
  line<-paste(variableName, "\t\t", cTotal[1], "\t",paste(cTotal[2],"(",cTotal[3],")", sep=""),"\t" , sep="") 
  it<-1
  for(iStudy in c("S1", "S2", "S3", "S4","S5") )
  {
    if(iStudy %in% data$study)
    {
      
      n<- data[data$study == iStudy,]$N[1]
      meanV<- data[data$study == iStudy,]$meanValue[1]
      sdV<- data[data$study == iStudy,]$sdValue[1]
      
      line<-paste(line, n,"\t", paste(meanV,"(",sdV,")\t", sep=""), sep="") 
    }
    else
    {line<-paste(line, "\t\t", sep ="")}  
    
  }
  cat(line, file=ffile, append=TRUE, sep = "\n")
}
util.printOneVariableMeanSd<-function(ffile1, data , varNameY, missingDataColName)
{
  
  #  summ <- paste0('mean(', varNameY, ', na.rm = TRUE)')  # construct summary method, e.g. mean(mpg)
  #  summ_name <- "varName1"
  #  rawDataX <- rawdata[!is.na(rawdata[,varNameY]) & rawdata[,missingDataColName] == "OK",] %>%
  #    group_by_(.dots = c("SUBJ_ID", "study")) %>%
  #    summarise_(.dots = setNames(summ, summ_name))  
  rm (rawDataX)
  rawDataX<- data[!is.na(data[,varNameY]) & data[,missingDataColName] == "OK",] %>% 
    group_by(SUBJ_ID, study) %>%
    summarise(varName1 = mean(!!sym(varNameY), na.rm = TRUE))
  
  rawDataX<- data[!is.na(data[,varNameY]) & data[,missingDataColName] == "OK",] %>% 
    group_by(SUBJ_ID, study) %>%
    summarise(varName1 = mean(!!sym(varNameY), na.rm = TRUE))
  
  pMean<-round(mean(rawDataX$varName1, na.rm = TRUE),2)
  pSd<- round(sd(rawDataX$varName1, na.rm = TRUE),2)
  pN<- length(unique(rawDataX$SUBJ_ID))
  rawDataX<- rawDataX %>%
    group_by(study) %>%
    summarise(meanValue = round(mean(varName1, na.rm = TRUE),2), 
              sdValue = round(sd(varName1 , na.rm = TRUE),2), 
              N = length(varName1))
  
  util.printDataRowMeanSD(ffile=ffile1 , data = rawDataX, variableName = varNameY, cTotal = c(pN, pMean, pSd ))
  
}
util.printDataRowPctCategories<-function(ffile , data , variableName )
{
  lineH<-paste(variableName, "\t\t\t\t" , sep="") ### top empty line
  cat(lineH, file=ffile, append=TRUE, sep = "\n")
  line<-""
  allN<-sum(data$N)
  for(oneEthnicity in unique(data$ETHNICITY))
  {
    totN<-sum(data[data$ETHNICITY == oneEthnicity ,]$N)
    totPct<-paste(round(100 * totN / allN , 2), "%", sep="")
    line<-paste(line,"\t",oneEthnicity,"\t",totN,"\t",totPct,"\t", sep="")    
    
    for(iStudy in c("S1", "S2", "S3", "S4","S5") )
    {
      
      if(nrow(data[data$ETHNICITY == oneEthnicity & data$study ==iStudy,])>0)
      {
        n<- data[data$ETHNICITY == oneEthnicity & data$study ==iStudy,]$N[1]
        pct<-data[data$ETHNICITY == oneEthnicity & data$study ==iStudy,]$pct[1]
        line<-paste(line, n,"\t", pct,"\t", sep="") 
      }
      else
      {line<-paste(line, "\t\t", sep ="")}  
    }
...
util.printDataRowBART<-function(ffile , data1, data2 , variableName )
{
  lineH<-paste(variableName, "\t\t\t\t" , sep="") ### top empty line
  cat(lineH, file=ffile, append=TRUE, sep = "\n")
  line<-""
  
  for(oneBlock in unique(data2$values.blockname))
  {
    totN<-data1[data1$values.blockname == oneBlock , ]$n[1]
    totMean<-round(data1[data1$values.blockname == oneBlock , ]$bartScoreMean[1],2)
    totSd<- round(data1[data1$values.blockname == oneBlock , ]$bartScoreSd[1],2)
    totMeanSd<-paste(totMean,"(",totSd,")", sep="")
    line<-paste(line,"\t",oneBlock,"\t",totN,"\t",totMeanSd,"\t", sep="")    
    
    for(iStudy in c("S1", "S2", "S3", "S4","S5") )
    {
      
      if(nrow(data2[data2$values.blockname == oneBlock & data2$study ==iStudy,])>0)
      {
        n<- data2[data2$values.blockname == oneBlock & data2$study ==iStudy,]$n[1]
        m<- round(data2[data2$values.blockname == oneBlock & data2$study ==iStudy,]$bartScoreMean[1],2)
        sd<-round(data2[data2$values.blockname == oneBlock & data2$study ==iStudy,]$bartScoreSd[1],2)
        meanSd<-paste(m,"(",sd,")", sep="")
        line<-paste(line, n,"\t", meanSd,"\t", sep="") 
      }
      else
      {line<-paste(line, "\t\t", sep ="")}  
    }
...
util.GetExcludedSubjectIDs_NOTS4_v4<-function(data)
{
  resultDf<-data#[!data$STUDY %in% c("S4"), ]
  resultDf<-resultDf[resultDf$AGE < 18| resultDf$BMI <= 16.5 | resultDf$SUBJ_ID %in% 
                       c("S7_135" , "S7_129" , "S7_114" , "S7_110"), ]
...
util.GetExcludedSubjectIDs_NOTS4_v3<-function(data)
{
  resultDf<-data#[!data$STUDY %in% c("S4"), ]
  resultDf<-resultDf[resultDf$AGE < 18 | resultDf$BMI <= 16.5 | resultDf$BMI >= 30 | resultDf$EDE_Q_Total >4 | resultDf$EDE_Q_Restrain >4, ]
  return(unique(resultDf$SUBJ_ID))
}

util.GetExcludedSubjectIDs_NOTS4_v2<-function(data)
{
  resultDf<-data#[!data$STUDY %in% c("S4"), ]
  resultDf<-resultDf[resultDf$AGE < 18 | resultDf$BMI <= 16.5 | resultDf$BMI >= 30| resultDf$EDE_Q_Total >4, ]
  return(unique(resultDf$SUBJ_ID))
}
## remove S4_71 for Anxiety Disorder / 
util.GetExcludedSubjectIDs_S4<-function(data)
{
  resultDf<-data#[data$STUDY %in% c("S4"), ]
  #resultDf<-resultDf[resultDf$AGE < 18 | resultDf$SUBJ_ID %in% c("S4_22", "S4_37", "S4_27" , "S4_69" , "S4_70"), ]
  #resultDf<-resultDf[resultDf$AGE < 18 | resultDf$SUBJ_ID %in% c("S4_22", "S4_37" , "S4_71" ), ]
  ...
  # S4_34 (RE) re for only 2 months: BUT we use her as she has actually recovered longer than that.
  #"S4_27" , "S4_69" , "S4_70" are recovered less than 6months ago
  # S4_71 (HC) has anxiety disorder
  # S8_47 RE already participated as AN previously
  # S4_6 RE no EDEQ and other questionnaire data 
  # S8_11 AN 
  # S8_48 MALE
